  ...
  if (lines.length === 1) {
    ...
    lines.forEach((line, index) => {
      if (index === 0) {
        ...
        result.push(
          `${delimiters.beginning[0]} ${line} ${delimiters.beginning[1]}`
        );
      } else if (index === lines.length - 1) {
        // last line of text, pad and use end delimiters
        result.push(
          `${delimiters.end[0]} ${line.padEnd(longestLine, ' ')} ${
            delimiters.end[1]
          }`
        );
      } else {
        // all other lines, use middle delimiters
        result.push(`${delimiters.middle[0]} ${line} ${delimiters.middle[1]}`);
      }
    });
  }
...
module.exports.say = (message, lineWidth = 80) => {
  ...
  const wrapped = wordWrap(message, { width: lineWidth - 4 });
